...
type RagMethod = 'bm25' | 'dense' | 'hybrid';
type RerankerMode = 'none' | 'jina-reranker-v2';
...
async function start(): Promise<void> {
  await loadDataset();
  await loadEmbeddingCache(defaultEmbeddingModel);

  if (process.argv.includes('--build-index')) {
    await buildDenseIndex(defaultEmbeddingModel);
    console.log(`Dense index sẵn sàng: ${documents.length} chunks, model ${defaultEmbeddingModel}.`);
    return;
  }

  if (process.argv.includes('--smoke')) {
    const question = examples[0]?.question || 'Hồ sơ bệnh án nào cần được hoàn thiện?';
    const hits = retrieveBm25(question, 3);
    console.log(JSON.stringify({
      datasetExamples: examples.length,
      evaluationExamples: evaluationReferences.size,
      fewShotExamples: fewShotExamples.length,
      evidenceChunks: documents.length,
      question,
      topHits: hits,
    }, null, 2));
    return;
